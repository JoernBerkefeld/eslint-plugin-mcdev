...
export function runValidation(filename, text, runtimeOptions = {}) {
    ...
    const validationConfig = rc
        ? resolveValidationConfig(rc.config, info.type, runtimeOptions.method)
        : null;
    if (!validationConfig) {
        return [];
    }
    ...
    for (const [ruleName, rule] of Object.entries(validationRules)) {
        ...
        if (passed === false) {
            messages.push({
                ruleId: `mcdev/${ruleName}`,
                // "fix" is treated as "error" because this plugin does not run mcdev's autofix.
                severity: severityString === 'error' || severityString === 'fix' ? 2 : 1,
...
/**
 * Resolves the effective validation config for a metadata type. When an explicit method is
 * requested (e.g. from tests) it is used as-is. Otherwise the plugin prefers the dedicated
 * `eslint` method and falls back to `deploy` (the strictest built-in method).
 *
 * @param {object} rcConfig parsed .mcdevrc.json content
 * @param {string} type mcdev metadata type used to apply overrides
 * @param {string} [method] explicit validation method override
 * @returns {import('./mcdevrc.js').ValidationConfig|null} merged rule->severity map, or null
 */
function resolveValidationConfig(rcConfig, type, method) {
    if (method) {
        return getValidationConfig(rcConfig, method, type);
    }
    return (
        getValidationConfig(rcConfig, 'eslint', type) ??
        getValidationConfig(rcConfig, 'deploy', type)
    );
}
